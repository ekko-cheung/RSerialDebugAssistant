...
- **Modbus RTU** - 支持线圈（01/05/0F）及保持/输入寄存器（03/04/06/10）的读写，自动处理 CRC-16 和响应解析
...
6. **使用 Modbus RTU**
   - 在发送面板切换到 **Modbus** 模式
   - 填写从站地址、PDU 起始地址和功能码
   - 线圈支持读、写单个和写多个；寄存器支持读保持/输入寄存器，以及写单个/多个保持寄存器
   - 地址按 Modbus PDU 的 0 起始地址填写；写入值支持十进制和 0x 十六进制
   - 请求帧会自动追加 Modbus RTU CRC-16，响应会校验并显示在面板和通信日志中
